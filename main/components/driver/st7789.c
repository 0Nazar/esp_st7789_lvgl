...
void spi_master_init(TFT_t * dev, int16_t mosi_pin, int16_t clk_pin, int16_t cs_pin, int16_t dc_pin, int16_t rst_pin, int16_t bl_pin)
{
	esp_err_t ret;
	...
	ESP_LOGI(TAG, "PIN_NUM_DC=%d",PIN_NUM_DC);
	//gpio_pad_select_gpio( PIN_NUM_DC );
	gpio_reset_pin( PIN_NUM_DC );
	gpio_set_direction( PIN_NUM_DC, GPIO_MODE_OUTPUT );
	gpio_set_level( PIN_NUM_DC, 0 );

	ESP_LOGI(TAG, "PIN_NUM_RST=%d",PIN_NUM_RST);
	if ( PIN_NUM_RST >= 0 ) {
	...
	ESP_LOGI(TAG, "PIN_NUM_BL=%d",PIN_NUM_BL);
	if ( PIN_NUM_BL >= 0 ) {
		//gpio_pad_select_gpio(PIN_NUM_BL);
...
void lcdDrawBufWithOffset(TFT_t * dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t *buffer) {  

    uint16_t height0 = y2 - y1;
    uint16_t width0 = x2 - x1;
	
    for (int16_t i = 0; i <= height0; i++){
	
		spi_master_write_command(dev, 0x2A);
        spi_master_write_addr(dev, x1, x2); 
        spi_master_write_command(dev, 0x2B);
        spi_master_write_addr(dev, y1 + i, y1 + i);  
        spi_master_write_command(dev, 0x2C);  
        spi_master_write_colors(dev, buffer + (i * (1 + width0)),1 + width0);    
    }
